...
public class CornerRadiusTransform implements Transform {
  ...
  @Override public void onDraw(Canvas canvas, Paint paint, Bitmap buffer) {
    if (mCornerRadius == 0) {
    ...
    if (mShader == null) {
      ...
      shaderMatrix.preScale(mDstRectF.width() / buffer.getWidth(),
          mDstRectF.height() / buffer.getHeight());
