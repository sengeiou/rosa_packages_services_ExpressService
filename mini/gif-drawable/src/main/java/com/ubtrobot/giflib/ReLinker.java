...
class ReLinker {
  private static final String MAPPED_BASE_LIB_NAME =
      System.mapLibraryName(LibraryLoader.BASE_LIBRARY_NAME);
  ...
  private static File unpackLibrary(final Context context) {
    ...
    final String mappedSurfaceLibraryName =
        System.mapLibraryName(LibraryLoader.SURFACE_LIBRARY_NAME);
    final FilenameFilter filter = new FilenameFilter() {
      @Override public boolean accept(File dir, String filename) {
        return filename.startsWith(MAPPED_BASE_LIB_NAME) || filename.startsWith(
            mappedSurfaceLibraryName);
      }
    };
      ...
      while (tries++ < MAX_TRIES) {
        ...
        if (libraryEntry == null) {
          throw new IllegalStateException(
              "Library " + MAPPED_BASE_LIB_NAME + " for supported ABIs not found in APK file");
        }
