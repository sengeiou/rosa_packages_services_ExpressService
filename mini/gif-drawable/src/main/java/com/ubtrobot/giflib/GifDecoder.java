 ...
 * For access only metadata (size, number of frames etc.) without pixels see {@link
...
public class GifDecoder {
   ...
   * @throws IOException when creation fails
   */
  public GifDecoder(@NonNull final InputSource inputSource, @Nullable final GifOptions options)
      throws IOException {
   ...
   */
  public void seekToTime(@IntRange(from = 0, to = Integer.MAX_VALUE) final int position,
      @NonNull final Bitmap buffer) {
    checkBuffer(buffer);
   ...
   */
  public GifInfoHandle getHandle() {
  ...
  private void checkBuffer(final Bitmap buffer) {
    ...
    if (buffer.getWidth() < mGifInfoHandle.getWidth()
        || buffer.getHeight() < mGifInfoHandle.getHeight()) {
