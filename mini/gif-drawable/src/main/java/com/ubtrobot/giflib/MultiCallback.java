...
public class MultiCallback implements Callback {
  ...
  /**
   * Set <code>useViewInvalidate</code> to <code>true</code> if displayed {@link Drawable} is not
   * supported by
   * {@link Callback#invalidateDrawable(Drawable)} of the target. For example if it is located
   * inside {@link android.text.style.ImageSpan}
   * displayed in {@link TextView}.
   *
   * @param useViewInvalidate whether {@link View#invalidate()} should be used instead of {@link
  ...
  @Override public void scheduleDrawable(@NonNull final Drawable who, @NonNull final Runnable what,
      final long when) {
    for (int i = 0; i < mCallbacks.size(); i++) {
      final CallbackWeakReference reference = mCallbacks.get(i);
      final Callback callback = reference.get();
  ...
  static final class CallbackWeakReference extends WeakReference<Callback> {
    ...
    @Override public boolean equals(final Object o) {
      return this == o
          ...
          && get() == ((CallbackWeakReference) o).get();
    }
