...
final class GifViewUtils {
  ...
  static GifImageViewAttributes initImageView(ImageView view, AttributeSet attrs, int defStyleAttr,
    ...
    if (attrs != null && !view.isInEditMode()) {
      final GifImageViewAttributes viewAttributes =
          new GifImageViewAttributes(view, attrs, defStyleAttr, defStyleRes);
    ...
    if (uri != null) {
      try {
        imageView.setImageDrawable(
            new GifDrawable(imageView.getContext().getContentResolver(), uri));
        return true;
  ...
  static class GifViewAttributes {
    ...
    GifViewAttributes(final View view, final AttributeSet attrs, final int defStyleAttr,
      ...
      final TypedArray gifViewAttributes = view.getContext()
          .obtainStyledAttributes(attrs, R.styleable.GifView, defStyleAttr, defStyleRes);
  ...
  static class GifImageViewAttributes extends GifViewAttributes {
    ...
    GifImageViewAttributes(final ImageView view, final AttributeSet attrs, final int defStyleAttr,
        final int defStyleRes) {
    ...
    private static int getResourceId(ImageView view, AttributeSet attrs, final boolean isSrc) {
      final int resId =
          attrs.getAttributeResourceValue(ANDROID_NS, isSrc ? "src" : "background", 0);
      if (resId > 0) {
        final String resourceTypeName = view.getResources().getResourceTypeName(resId);
        if (SUPPORTED_RESOURCE_TYPE_NAMES.contains(resourceTypeName) && !setResource(view, isSrc,
            resId)) {
          return resId;
        }
      }
