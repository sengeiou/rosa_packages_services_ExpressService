...
/**
 * Lightweight version of {@link com.ubtrobot.giflib.GifDrawable} used to retrieve metadata of GIF
 * only,
...
public class GifAnimationMetaData implements Serializable, Parcelable {
   ...
   */
  public GifAnimationMetaData(@NonNull Resources res, @RawRes @DrawableRes int id)
   ...
   */
  public GifAnimationMetaData(@NonNull AssetManager assets, @NonNull String assetName)
      throws IOException {
  ...
  /**
   * Like {@link GifDrawable#getAllocationByteCount()} but does not include memory needed for
   * backing {@link android.graphics.Bitmap}.
   * {@code Bitmap} in {@code GifDrawable} may be allocated at the time of creation or existing one
   * may be reused if {@link GifDrawableBuilder#with(GifDrawable)}
   * is used.
   ...
   * {@link #getDrawableAllocationByteCount(GifDrawable, int)}
   * can be used.
   *
   * @return possible size of the memory needed to store pixels excluding backing {@link
   * android.graphics.Bitmap} and assuming no subsampling
   */
  ...
  /**
   * Like {@link #getAllocationByteCount()} but includes also backing {@link
   * android.graphics.Bitmap} and takes sample size into account.
   *
   ...
   */
  public long getDrawableAllocationByteCount(@Nullable GifDrawable oldDrawable,
      @IntRange(from = 1, to = Character.MAX_VALUE) int sampleSize) {
    if (sampleSize < 1 || sampleSize > Character.MAX_VALUE) {
      throw new IllegalStateException(
          "Sample size " + sampleSize + " out of range <1, " + Character.MAX_VALUE + ">");
    }
  ...
  @Override public String toString() {
    ...
    final String suffix =
        String.format(Locale.ENGLISH, "GIF: size: %dx%d, frames: %d, loops: %s, duration: %d",
