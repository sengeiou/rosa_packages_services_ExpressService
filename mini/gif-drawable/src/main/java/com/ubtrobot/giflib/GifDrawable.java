...
public class GifDrawable extends Drawable implements Animatable, MediaPlayerControl,
   ...
   *
   * @param inputSource The {@link InputSource} concrete subclass used to construct {@link
   * GifDrawable}.
   ...
   * @param isRenderingTriggeredOnDraw True if rendering of the next frame is scheduled after
   * drawing current one, false otherwise.
   * @param options Options controlling various GIF parameters.
   * @throws IOException if input source is invalid.
   */
  protected GifDrawable(@NonNull InputSource inputSource, @Nullable GifDrawable oldDrawable,
      @Nullable ScheduledThreadPoolExecutor executor, boolean isRenderingTriggeredOnDraw,
  ...
  public GifDrawable(GifInfoHandle gifInfoHandle, final GifDrawable oldDrawable,
    ...
    if (oldBitmap == null) {
      mBuffer = Bitmap.createBitmap(mNativeInfoHandle.getWidth(), mNativeInfoHandle.getHeight(),
   ...
   */
  @Nullable public static GifDrawable createFromResource(@NonNull Resources res,
  ...
  @Override public void stop() {
    ...
    //Fixme logic.peng animation listener call
    for (AnimationListener listener: mListeners) {
      listener.onAnimationCompleted(mNativeInfoHandle.getLoopCount());
    }
  }
  ...
  @Override public String toString() {
    return String.format(Locale.ENGLISH, "GIF: size: %dx%d, frames: %d, error: %d",
        ...
        mNativeInfoHandle.getNumberOfFrames(), mNativeInfoHandle.getNativeErrorCode());
  }
  ...
  private volatile boolean seekRefresh = false;
  ...
  public void getPixels(@NonNull int[] pixels) {
    mBuffer.getPixels(pixels, 0, mNativeInfoHandle.getWidth(), 0, 0, mNativeInfoHandle.getWidth(),
        mNativeInfoHandle.getHeight());
  }
  ...
  public int getPixel(@IntRange(from = 0) int x, @IntRange(from = 0) int y) {
    if (x
        >= mNativeInfoHandle.getWidth()) { //need to check explicitly because reused bitmap may be larger
  ...
  /**
   * Removes all animation listener
   */
