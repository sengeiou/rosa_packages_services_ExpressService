...
Java_com_ubtrobot_giflib_GifInfoHandle_reset(JNIEnv *__unused  env, jclass  __unused class,
                                             jlong gifInfo) {
...
Java_com_ubtrobot_giflib_GifInfoHandle_setSpeedFactor(JNIEnv __unused *env,
...
__unused JNIEXPORT void JNICALL
Java_com_ubtrobot_giflib_GifInfoHandle_seekToTime(JNIEnv *env, jclass __unused handleClass,
...
__unused JNIEXPORT void JNICALL
Java_com_ubtrobot_giflib_GifInfoHandle_seekToFrame(JNIEnv *env, jclass __unused handleClass,
                                                   jlong gifInfo, jint desiredIndex,
                                                   jobject jbitmap) {
    GifInfo *info = (GifInfo *) (intptr_t) gifInfo;
    if (info == NULL || info->gifFilePtr->ImageCount == 1) {
        return;
...
__unused JNIEXPORT void JNICALL
Java_com_ubtrobot_giflib_GifInfoHandle_saveRemainder(JNIEnv *__unused  env,
                                                     jclass __unused handleClass,
                                                     jlong gifInfo) {
    GifInfo *info = (GifInfo *) (intptr_t) gifInfo;
    if (info == NULL || info->lastFrameRemainder != -1 ||
        info->currentIndex == info->gifFilePtr->ImageCount ||
...
Java_com_ubtrobot_giflib_GifInfoHandle_restoreRemainder(JNIEnv *__unused env,
                                                        jclass __unused handleClass,
                                                        jlong gifInfo) {
    GifInfo *info = (GifInfo *) (intptr_t) gifInfo;
