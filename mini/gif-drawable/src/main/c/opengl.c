...
Java_com_ubtrobot_giflib_GifInfoHandle_glTexImage2D(JNIEnv *__unused unused,
...
__unused JNIEXPORT void JNICALL
Java_com_ubtrobot_giflib_GifInfoHandle_glTexSubImage2D(JNIEnv *__unused env,
                                                       jclass __unused handleClass, jlong gifInfo,
                                                       jint target, jint level) {
    GifInfo *info = (GifInfo *) (intptr_t) gifInfo;
    if (info == NULL || info->frameBufferDescriptor == NULL) {
...
static void *slurp(void *pVoidInfo) {
    ...
    while (1) {
        ...
        if (pollResult < 0) {
            ...
            const int readResult = TEMP_FAILURE_RETRY(
                    eventfd_read(texImageDescriptor->eventPollFd.fd, &eventValue));
            if (readResult != 0) {
...
__unused JNIEXPORT void JNICALL
Java_com_ubtrobot_giflib_GifInfoHandle_initTexImageDescriptor(JNIEnv *env,
...
__unused JNIEXPORT void JNICALL
Java_com_ubtrobot_giflib_GifInfoHandle_startDecoderThread(JNIEnv *env, jclass __unused handleClass,
                                                          jlong gifInfo) {
    GifInfo *info = (GifInfo *) (intptr_t) gifInfo;
    if (info == NULL) {
        return;
...
__unused JNIEXPORT void JNICALL
Java_com_ubtrobot_giflib_GifInfoHandle_stopDecoderThread(JNIEnv *env, jclass __unused handleClass,
                                                         jlong gifInfo) {
    GifInfo *info = (GifInfo *) (intptr_t) gifInfo;
    if (info == NULL || info->frameBufferDescriptor == NULL) {
        return;
...
__unused JNIEXPORT void JNICALL
Java_com_ubtrobot_giflib_GifInfoHandle_seekToFrameGL(__unused JNIEnv *env,
                                                     jclass __unused handleClass, jlong gifInfo,
                                                     jint desiredIndex) {
    GifInfo *info = (GifInfo *) (intptr_t) gifInfo;
    if (info == NULL) {
