...
Java_com_ubtrobot_giflib_GifInfoHandle_bindSurface(JNIEnv *env, jclass __unused handleClass,
                                                   jlong gifInfo,
    ...
    while (true) {
        ...
        if (pollResult == 0)
            ...
            const int readResult = TEMP_FAILURE_RETRY(
                    eventfd_read(descriptor->eventPollFd.fd, &eventValue));
            if (readResult != 0) {
    ...
    if (ANativeWindow_setBuffersGeometry(window, (int32_t) gifFilePtr->SWidth,
                                         (int32_t) gifFilePtr->SHeight, windowFormat) != 0) {
    ...
    while (true) {
        pollResult = TEMP_FAILURE_RETRY(
                poll(&descriptor->eventPollFd, 1, (int) invalidationDelayMillis));
        ...
        invalidationDelayMillis = calculateInvalidationDelay(info, renderingStartTime,
                                                             frameDuration);
...
__unused JNIEXPORT void JNICALL
Java_com_ubtrobot_giflib_GifInfoHandle_postUnbindSurface(JNIEnv *env, jclass __unused handleClass,
                                                         jlong gifInfo) {
    GifInfo *info = (GifInfo *) (intptr_t) gifInfo;
