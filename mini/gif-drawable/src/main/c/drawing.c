...
static inline void disposeFrameIfNeeded(argb *bm, GifInfo *info) {
    ...
    bool nextTrans =
            info->controlBlock[info->currentIndex].TransparentColor != NO_TRANSPARENT_COLOR;
    uint_fast8_t nextDisposal = info->controlBlock[info->currentIndex].DisposalMode;

    if ((curDisposal == DISPOSE_PREVIOUS || nextDisposal == DISPOSE_PREVIOUS) &&
        info->backupPtr == NULL) {
    ...
    if (nextTrans || !checkIfCover(next, cur)) {
        if (curDisposal == DISPOSE_BACKGROUND || (info->currentIndex == 1 && curDisposal ==
                                                                             DISPOSE_PREVIOUS)) {// restore to background (under this image) color
            uint32_t *dst = (uint32_t *) GET_ADDR(bm, info->stride, cur->ImageDesc.Left,
                                                  cur->ImageDesc.Top);
        ...
        } else if (curDisposal == DISPOSE_PREVIOUS &&
                   nextDisposal == DISPOSE_PREVIOUS) {// restore to previous
            argb *tmp = bm;
            bm = backup;
            backup = tmp;
        }
...
void prepareCanvas(const argb *bm, GifInfo *info) {
    ...
    if (gifFilePtr->SColorMap && info->controlBlock->TransparentColor == NO_TRANSPARENT_COLOR) {
        ...
        argb *pixel;
        for (pixel = (argb *) bm;
             pixel < bm + (info->stride * info->gifFilePtr->SHeight); pixel++) {
            pixel->alpha = 0xFF;
