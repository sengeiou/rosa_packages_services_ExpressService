...
public class LottieComposition {
  ...
  private final PerformanceTracker performanceTracker = null/*new PerformanceTracker()*/;
  ...
  public void tween(float progress) {
    for (int i = 0 ; i < layers.size(); ++i) {
      layers.get(i).tween(progress);
    }

    Iterator<String> ids = precomps.keySet().iterator();
    while (ids.hasNext()) {
      List<Layer> layerList = precomps.get(ids.next());
      for (int i = 0 ; i < layerList.size(); ++i) {
        layerList.get(i).tween(progress);
      }
    }
  }

  public boolean lessPrecomps() {
    return (precomps.size() <= 2 && bounds.width() == 400) || (precomps.size() <= 1 && bounds.width() == 240);
  }
