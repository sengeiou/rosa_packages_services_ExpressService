...
public class Layer {
  ...
  public Layer(List<ContentModel> shapes, LottieComposition composition, String layerName, long layerId,
    ...
    try {
      this.componentType = ComponentType.valueOf(layerName.trim().replace(".png", ""));
    }catch (Exception e) {
      //测试打开
      //Log.v("Logic",  "No enum constant " + layerName);
    }
  }
  ...
  public void tween(float progress) {
    if (CompomentUtil.isAnimatableCompoment(componentType)) {
      for (int i = 0; i < shapes.size(); ++i) {
        shapes.get(i).tween(componentType, progress);
      }
      transform.tween(componentType, progress);
    }
  }
}
